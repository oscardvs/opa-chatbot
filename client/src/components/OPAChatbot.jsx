  ...
  return (
    <>
      ...
      <div className="flex flex-col min-h-screen bg-gradient-to-b from-purple-950 to-purple-900">
            ...
            <div className="space-y-6 max-w-6xl mx-auto">
              {messages.map((message, index) => (
                ...
                >
                  ...
                  <div className={`
                    flex flex-col ${message.isUser ? 'items-end' : 'items-start'} max-w-[80%] md:max-w-[70%]
                  `}>
                    <div className={`
                      rounded-2xl px-4 py-3 shadow-md group relative
                      ${message.isUser ? 'bg-purple-700 text-purple-100 rounded-br-none' : 'bg-purple-800/50 text-purple-100 rounded-bl-none border border-purple-700/50'}
                    ...
                    `}>
                      ...
                      <div className={`
                        absolute ${message.isUser ? 'left-0' : 'right-0'} -top-3 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center space-x-2 bg-purple-900/90 rounded-full shadow-md px-2 py-1
                      `}>
                        {/* You can add additional controls/icons here */}
                      </div>
                    </div>
                    <span className="text-xs text-purple-400 mt-1 px-2">{message.timestamp}</span>
                  </div>
                  {message.isUser && (
                    <div className="flex-shrink-0 bg-purple-700/50 rounded-full p-2 animate-pop">
        ...
        {/* Input form */}
        <div className="fixed bottom-0 left-0 right-0 bg-gradient-to-r from-purple-900 to-purple-800 border-t border-purple-700/50 backdrop-blur-md px-4 py-4 safe-bottom mb-8">
          <form onSubmit={handleSubmit} className="max-w-6xl mx-auto">
            <div className="flex flex-col space-y-2 md:space-y-0 md:flex-row md:space-x-4">
              <input
                ...
                className="flex-1 bg-purple-800/50 border border-purple-700/50 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all duration-200 text-purple-100 placeholder-purple-400"
              />
              <button
                type="submit"
                disabled={!input.trim() || isLoading}
                className="bg-purple-700 text-purple-100 rounded-xl px-6 py-2 hover:bg-purple-600 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 focus:ring-offset-purple-900 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 transform hover:scale-105 active:scale-95 flex items-center justify-center"
              >
